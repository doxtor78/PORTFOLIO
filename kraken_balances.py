...
def get_prices(symbols):
    ...
    # Separate stablecoins/fiat and assign them a fixed price of 1.0
    stablecoins = ['USD', 'USDT', 'USDC', 'DAI', 'BUSD', 'TUSD', 'USDP', 'FDUSD', 'UST']
    for symbol in set(symbols):
        if symbol in stablecoins:
            prices[symbol] = 1.0
        elif symbol and symbol.isalpha() and symbol.isupper() and len(symbol) <= 10:
            # Collect valid crypto symbols to fetch from the API
            crypto_symbols_to_fetch.append(symbol)

    if not crypto_symbols_to_fetch:
        return prices # Return early if only stablecoins were passed

    headers = {'X-CMC_PRO_API_KEY': COINMARKETCAP_API_KEY}
    params = {'symbol': ','.join(set(crypto_symbols_to_fetch))}
    try:
        ...
        if response_json.get('status', {}).get('error_code') == 0 and 'data' in response_json:
            ...
            for symbol_upper in data:
                ...
                # Map the price back to all original symbols that match
                for original_symbol in crypto_symbols_to_fetch:
                    if original_symbol.upper() == symbol_upper:
                        prices[original_symbol] = price
        return prices
    except Exception as e:
        print(f"Error fetching prices from CoinMarketCap: {e}")
        # Return whatever prices we already have (stablecoins)
        return prices
